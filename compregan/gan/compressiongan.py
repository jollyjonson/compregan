...
from compregan.losses.distortions import MSE, Distortion
...
class CompressionGAN:
    ...
    def __init__(
        ...
        decoding_generator: tf.keras.Model,
        discriminator: tf.keras.Model,
        loss_terms: Union[LossTerm, Tuple[LossTerm, ...]] = MSE(
            mse_multiplier=10.0
        ),
        quantizer: tf.keras.Model = tf.identity,
        gan_loss: GANLoss = LeastSquares(),
        ...
        conditional: Optional[
            Union[tf.keras.Model, Callable, Conditional]
        ] = None,
        use_conditional: bool = True,
        encoder_optimizer: tf.keras.optimizers.Optimizer = Adam(),
        decoding_generator_optimizer: tf.keras.optimizers.Optimizer = Adam(),
        ...
        self._loss_terms = (
            (loss_terms,)
            if issubclass(type(loss_terms), LossTerm)
            else loss_terms
        )
        ...
        self._conditional_has_needed_components = issubclass(
            type(self._conditional), Conditional
        )
        self._using_conditional_gan = (
            self._conditional is not None and use_conditional
        )

        self._noise_prior = noise_prior
        self._noise_dim = (noise_dim,) if type(noise_dim) is int else noise_dim
        self._concatenating_latent_repr_with_noise = (
            self._noise_prior is not None
            and all(n > 0 for n in self._noise_dim)
        )
        ...
        self._metrics = (
            ...
            if type(metrics) is not tuple and type(metrics) is not list
            else metrics
        )

        self._auxillary_loss_terms = (
            auxillary_loss_terms
            if auxillary_loss_terms is not None
            else list()
        )
        self._auxillary_metrics = (
            auxillary_metrics if auxillary_metrics is not None else list()
        )
    ...
    @tf.function
    def _get_codec_data(
        self, data_batch: Union[tf.Tensor, tf.data.Dataset]
    ) -> tf.Tensor:
    ...
    def train(
        ...
        for epoch_idx in range(start_epoch_idx, num_epochs + 1):
            ...
            num_train_steps = (
                batched_dataset.cardinality().numpy()
            ...
            for batch_idx, batch in enumerate(batched_dataset):
                ...
                if (
                    not batch_idx == num_train_steps - 1
                    or validation_data is None
                ):  # spare last update if validation
                    progress_bar.update(
                        current=batch_idx + 1,
                        values=[
                            (k, v) for k, v in monitored_quantities.items()
                        ],
                    )
            ...
            if validation_data is not None:
                monitored_quantities.update(
                    self._validation_step(
                        validation_data, validation_steps, batch_size
                    )
                )
                progress_bar.update(
                    current=batch_idx + 1,
    ...
    def _train_step(
        self, train_data_batch: tf.data.Dataset
    ) -> Dict[str, float]:
        ...
        for (
            ...
            monitored_quantities.items()
        ):  # convert tf.Tensors to python scalars
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if self._using_conditional_gan:
                conditional_input = (
                    [
                        component_dict[k]
                        for k in self._conditional.needed_components
                    ]
                    if self._conditional_has_needed_components
                    else [codec_data]
                )
                conditioning_information = self._conditional(
                    *conditional_input
                )
            ...
            decoding_generator_inputs = (
                [latents, conditioning_information]
                if self._using_conditional_gan
                else latents
            )
            decoding_generator_outputs = self._decoding_generator(
                decoding_generator_inputs, training=True
            )
            ...
            discr_input_reconstr = (
                [reconstructed_data, conditioning_information]
                if self._using_conditional_gan
                else reconstructed_data
            )
            discr_out_reconstructed_data = self._discriminator(
                discr_input_reconstr, training=True
            )
            discr_input_real = (
                [codec_data, conditioning_information]
                if self._using_conditional_gan
                else codec_data
            )
            discr_out_real_data = self._discriminator(
                discr_input_real, training=True
            )

            # compute losses
            discr_loss = tf.reduce_mean(
                self._gan_loss.f(discr_out_real_data)
            ...
            for loss_term in self._loss_terms:
                needed_components = [
                    component_dict[key] for key in loss_term.needed_components
                ]
                loss_term_value = loss_term(*needed_components)
                monitored_quantities[loss_term.get_key()] = loss_term_value
                gen_loss, discr_loss = loss_term.apply(
                    loss_term_value, gen_loss, discr_loss
                )

            # loss terms for auxillary output
            for auxillary_output_idx, auxillary_output in enumerate(
                auxillary_outputs
            ):
                component_dict[Components.AuxillaryOutput] = auxillary_output
                for auxillary_loss_term in self._auxillary_loss_terms[
                    auxillary_output_idx
                ]:
                    needed_components = [
                        component_dict[key]
                        for key in auxillary_loss_term.needed_components
                    ]
                    auxillary_loss_term_value = auxillary_loss_term(
                        *needed_components
                    )
                    monitored_quantities[
                        auxillary_loss_term.get_key()
                    ] = auxillary_loss_term_value
                    gen_loss, discr_loss = auxillary_loss_term.apply(
                        auxillary_loss_term_value, gen_loss, discr_loss
                    )

        # update encoder, quantizer, generator (decoder)
        gradient_encoder = tape.gradient(
            gen_loss, self._encoder.trainable_variables
        )
        self._encoder_optimizer.apply_gradients(
            zip(gradient_encoder, self._encoder.trainable_variables)
        )

        if self._quantizer_optimizer is not None:
            gradient_quantizer = tape.gradient(
                gen_loss, self._quantizer.trainable_variables
            )
            self._quantizer_optimizer.apply_gradients(
                zip(gradient_quantizer, self._quantizer.trainable_variables)
            )

        gradient_decoding_generator = tape.gradient(
            gen_loss, self._decoding_generator.trainable_variables
        )
        self._decoding_generator_optimizer.apply_gradients(
            zip(
                gradient_decoding_generator,
                self._decoding_generator.trainable_variables,
            )
        )

        # update discriminator
        gradient_discriminator = tape.gradient(
            discr_loss, self._discriminator.trainable_variables
        )
        self._discriminator_optimizer.apply_gradients(
            zip(
                gradient_discriminator, self._discriminator.trainable_variables
            )
        )

        # store losses
        monitored_quantities[
            self.QuantityNames.generator_loss
        ] = tf.reduce_mean(gen_loss)
        monitored_quantities[
            self.QuantityNames.discriminator_loss
        ] = tf.reduce_mean(discr_loss)

        monitored_quantities[
            self.QuantityNames.discriminator_acc
    ...
    def _validation_step(
        ...
        for batch_idx, batch in enumerate(batched_dataset):
            if batch_idx == 0:
                ...
                for (
                    ...
                    monitored_quantities_val.items()
                ):  # convert tf.Tensors to python scalars
                ...
                for (
                    ...
                    monitored_quantities_val[key] += float(
                        monitored_quantities_val_tmp[key].numpy()
                    )
        ...
        for (
            key,
            value,
        ) in monitored_quantities_val.items():  # compute mean over batches
            monitored_quantities_val[key] /= batch_idx + 1

        return monitored_quantities_val

    @tf.function
    def _on_validation_step(
        self, validation_data_instance: tf.Tensor
    ) -> Dict[str, tf.Tensor]:
        monitored_quantities = dict()
        ...
        # compute losses
        monitored_quantities[
            __val_name(self.QuantityNames.generator_loss)
        ] = tf.reduce_mean(
            self._gan_loss.f(discr_out_real_data)
        ) + tf.reduce_mean(
            self._gan_loss.g(discr_out_reconstructed_data)
        )
        monitored_quantities[
            __val_name(self.QuantityNames.discriminator_loss)
        ] = tf.reduce_mean(self._gan_loss.f(discr_out_reconstructed_data))
        ...
        for loss_term in self._loss_terms:
            needed_components = [
                component_dict[k] for k in loss_term.needed_components
            ]
            monitored_quantities[__val_name(loss_term.get_key())] = loss_term(
                *needed_components
            )

        for metric in self._metrics:
            monitored_quantities[__val_name(metric.get_key())] = metric(
                codec_data, reconstructed_data
            )
        ...
        for auxillary_output_idx, auxillary_output in enumerate(
            ...
            component_dict[Components.AuxillaryOutput] = auxillary_output
            for auxillary_loss_term_or_metric in [
                *self._auxillary_loss_terms[auxillary_output_idx],
                *self._auxillary_metrics[auxillary_output_idx],
            ]:
                needed_components = [
                    component_dict[key]
                    for key in auxillary_loss_term_or_metric.needed_components
                ]
                auxillary_loss_term_value = auxillary_loss_term_or_metric(
                    *needed_components
                )
                monitored_quantities[
                    __val_name(auxillary_loss_term_or_metric.get_key())
                ] = auxillary_loss_term_value

        monitored_quantities[
            __val_name(self.QuantityNames.discriminator_acc)
        ] = self.compute_discriminator_accuracy(
            discr_out_real_data, discr_out_reconstructed_data
        )
    ...
    def compute_discriminator_accuracy(
        self,
        discriminator_out_real_data: tf.Tensor,
        discriminator_out_reconstructed_data: tf.Tensor,
    ) -> tf.Tensor:
        return (
            tf.reduce_mean(
                keras.metrics.binary_accuracy(
                    tf.ones_like(discriminator_out_real_data),
                    tf.round(discriminator_out_real_data),
                )
            )
            + tf.reduce_mean(
                keras.metrics.binary_accuracy(
                    tf.zeros_like(discriminator_out_reconstructed_data),
                    tf.round(discriminator_out_reconstructed_data),
                )
            )
        ) / 2
    ...
    def encode_data(
        ...
        if self._using_conditional_gan and conditioning_information is None:
            ...
            self._using_conditional_gan
            and conditioning_information is not None
        ):
            encoder_input = [data, conditioning_information]
        else:
            encoder_input = data
        ...
        return (
            quantized_latent_repr.numpy()
            if return_np_array
            else quantized_latent_repr
        )

    def decode(
        self,
        encoded_representation: Union[tf.Tensor, np.ndarray],
        conditioning_information: Optional[tf.Tensor] = None,
        return_np_array: bool = True,
    ) -> Union[tf.Tensor, np.ndarray]:
        if self._concatenating_latent_repr_with_noise:
            noise = self._noise_prior(
                (encoded_representation.shape[0], *self._noise_dim)
            )
        ...
        if type(decoded_data) is tf.Tensor:
            decoded_data = (
                decoded_data.numpy() if return_np_array else decoded_data
            )
        else:
            decoded_data = (
                list(map(lambda t: t.numpy(), decoded_data))
                if return_np_array
                else decoded_data
            )
    ...
    def save_to_file(self, path: Union[str, os.PathLike]):
        ...
        tmp_dir = os.path.join(
            os.path.dirname(path), "tmpCompressionGAN_save_to_file"
        )
        os.mkdir(tmp_dir)

        try:
            # save all models separately and remove them from this instance
            # for pickling, save them in a dict for later
            models = dict()
            ...
            instance_save_path = os.path.join(
                tmp_dir, self._saved_instance_filename
            )
            ...
            with subprocess.Popen(
                [
                    "tar",
                    "-czf",
                    f"{os.path.basename(path)}.tar.gz",
                    "-C",
                    f"{tmp_dir}",
                    ".",
                ],
    ...
    def load_from_file(cls, path: Union[str, os.PathLike]):
        path = (
            ...
            else os.path.abspath(path) + ".tar.gz"
        )

        tmp_dir = os.path.join(
            os.path.dirname(path), "tmpCompressionGAN_load_from_file"
        )
        os.mkdir(tmp_dir)
        ...
        try:
            ...
            # load instance
            with open(
                os.path.join(tmp_dir, cls._saved_instance_filename), "rb"
            ) as pkl_file_handle:
                compression_gan_instance = pickle.load(
                    pkl_file_handle
                )  # type: CompressionGAN

            # any directory corresponds to a model,
            # load the models and again set them as members of the instance
    ...
    def _warn_if_ignoring_data(steps_per_epoch: int, shuffle: bool):
        if steps_per_epoch is not None and not shuffle:
            warnings.warn(
                f"Performing {steps_per_epoch} steps per epoch "
