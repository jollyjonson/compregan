...
class Distortion(LossTerm, metaclass=ABCMeta):
    """
    Interface for distortion loss terms. Assuming we need the original data as well as the reconstruction to
    ...
    @tf.function
    def apply(self, loss_value: tf.Tensor, generator_loss: tf.Tensor, discr_loss: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
