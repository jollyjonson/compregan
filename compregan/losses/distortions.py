...
class MSE(Distortion):
    ...
    def __call__(
        ...
        distortion_term = tf.reduce_mean(
            tf.keras.losses.MSE(data, reconstructed_data)
        )
...
class Lp(Distortion):
    ...
    def __call__(
        ...
        lp_distortion = tf.pow(
            tf.reduce_sum(
                tf.pow(
                    tf.cast(tf.abs(data - reconstructed_data), tf.float32),
                    self._p,
                )
            ),
            1.0 / self._p,
    ...
    def get_key(self) -> str:
        displayed_p = (
            int(self._display_p)
            if self._display_p % 1 == 0
            else self._display_p
        )
        return f"l{displayed_p}"
...
class MSSSIM(Distortion):
    ...
    def _map_to_0_1(image_in_minus_one_one: tf.Tensor) -> tf.Tensor:
        return (image_in_minus_one_one + 1.0) / 2.0

    @tf.function
    def __call__(
        self, data: tf.Tensor, reconstructed_data: tf.Tensor
    ) -> tf.Tensor:
        data, reconstructed_data = self._map_to_0_1(data), self._map_to_0_1(
            reconstructed_data
        )
        return tf.image.ssim_multiscale(data, reconstructed_data, max_val=1.0)
...
class PSNR(MSSSIM):
    ...
    def __call__(
        ...
        return tf.image.psnr(data, reconstructed_data, max_val=1.0)
