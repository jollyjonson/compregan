...
class AuxillaryLossTerms(list):
    ...
    loss_terms: List[List[LossTerm]]
        List of lists of instances of child classes of `LossTerm`.
        Each list of loss terms will be applied in the
        order that the auxillary output appears at the
        decoding_generator during `CompressionGAN.train`.
    """
...
class CategorialCrossEntropy(AuxLoss):
    @tf.function
    def __call__(
        self, prediction: tf.Tensor, complete_train_data: tf.data.Dataset
    ) -> tf.Tensor:
        ground_truth = complete_train_data[self._class_label_key]
        return tf.reduce_mean(
            tf.keras.losses.categorical_crossentropy(ground_truth, prediction)
        )
...
class CategorialAccuracy(AuxLoss):
    ...
    def __call__(
        ...
        ground_truth = complete_train_data[self._class_label_key]
        return tf.reduce_mean(
            tf.keras.metrics.categorical_accuracy(ground_truth, prediction)
        )
    ...
    def apply(
        ...
        raise ValueError(
            "Accuracy is not differentiable and can only be used as a metric"
        )
