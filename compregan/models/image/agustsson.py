...
class AgustssonImageModelsBuilder:
    def __init__(
        ...
        initializer: tf.keras.initializers.Initializer = tf.keras.initializers.glorot_normal,  # noqa
        normalization: tf.keras.layers.LayerNormalization = tfa.layers.InstanceNormalization,  # noqa
        activation: tf.keras.layers.Activation = tf.keras.layers.LeakyReLU,
        num_filters_per_layer_encoder: Tuple[int, ...] = (
            60,
            120,
            240,
            480,
        ...
        num_filter_per_layer_discriminator: Tuple[int, ...] = (
            64,
            128,
            256,
            512,
        ...
        assert (
            not multiply_latent_space_with_binary_heatmap
            if not self._use_sc_scheme
            else True
        )
        self._multiply_latent_space_with_binary_heatmap = (
            multiply_latent_space_with_binary_heatmap
        )
        assert (
            binary_heatmap_channel is not None
            if self._multiply_latent_space_with_binary_heatmap
            else True
        )
        ...
        if normalization == tf.keras.layers.BatchNormalization:
            ...
            raise NotImplementedError(
                f"No kwargs found for normalization type "
                f"'{type(normalization)}'"
            )
        ...
        num_filters_per_layer_encoder_default = (
            (60, 120, 240, 480, 960)
            if not use_sc_scheme
            else (60, 120, 240, 480)
        )
        self._num_filters_per_layer_encoder = (
            num_filters_per_layer_encoder
            if num_filters_per_layer_encoder is not None
            else num_filters_per_layer_encoder_default
        )
        self._num_filters_per_layer_decoder = num_filters_per_layer_decoder
        self._num_filters_per_layer_discriminator = (
            num_filter_per_layer_discriminator
        )
        ...
        self._decoding_generator_output_activation = (
            decoding_generator_output_activation
        )
    ...
    def _build_encoder(self) -> tf.keras.Model:
        ...
        SC: c7s1-60, d120, d240, d480, c3s1-C, q, (c3s1-480, d960)
        - layers in parenthesis are implemented in the decoder
        """

        image_input = tf.keras.Input(
            shape=(self._width, self._height, self._num_channels)
        )
        image_input_f32 = tf.cast(image_input, tf.float32)

        if self._use_sc_scheme:
            semantic_map_input = tf.keras.Input(
                shape=(self._width, self._height, self._num_semantic_classes)
            )
            semantic_map_input_f32 = tf.cast(semantic_map_input, tf.float32)
            encoder_input = tf.concat(
                [image_input_f32, semantic_map_input_f32], axis=-1
            )
        ...
        x = self._conv_block(
            x,
            num_filters=self._num_filters_per_layer_encoder[0],
            kernel_size=7,
            strides=1,
            padding="valid",
        )
        ...
        for num_filters in self._num_filters_per_layer_encoder[1:]:
            x = self._conv_block(
                x, num_filters=num_filters, kernel_size=3, strides=2
            )

        # Project channels onto space w/ dimension channels_latent_space
        # Feature maps have dimension
        # ceil(W/16) x ceil(H/16) x channels_latent_space
        x = tf.pad(x, [[0, 0], [1, 1], [1, 1], [0, 0]], "reflect")

        activation_temp = self._activation
        self._activation = lambda: lambda x: x
        feature_map = self._conv_block(
            x,
            num_filters=self._num_channels_latent_space,
            kernel_size=3,
            strides=1,
            padding="valid",
        )
    ...
    def _build_semantic_map_encoder_layers(
        self, model_input: tf.keras.layers.Layer
    ) -> tf.keras.layers.Layer:
        """c7s1-60, d120, d240, d480, d960"""
        x = tf.cast(model_input, tf.float32)
        x = tf.pad(x, [[0, 0], [3, 3], [3, 3], [0, 0]], "reflect")
        x = self._conv_block(
            x, num_filters=60, kernel_size=7, strides=1, padding="valid"
        )
    ...
    def _build_decoding_generator(self) -> tf.keras.Model:
        ...
        shape_latent_repr_with_noise = (
            *output_shape_encoder[:-1],
            output_shape_encoder[-1] + self._num_noise_channels,
        )
        quantized_latent_representation_and_noise = tf.keras.layers.Input(
            shape=shape_latent_repr_with_noise
        )
        inputs = [quantized_latent_representation_and_noise]

        if self._use_sc_scheme:
            intermediate_latents = self._conv_block(
                quantized_latent_representation_and_noise, 480, 3, 1
            )
            intermediate_latents = self._conv_block(intermediate_latents, 960)

            # TODO [AgustssonModelBuilder]: SC model
            #  add pointwise binary heatmap multiplication

            semantic_label_map_input = tf.keras.layers.Input(
                shape=(self._width, self._height, self._num_semantic_classes)
            )
            inputs.append(semantic_label_map_input)
            semantic_label_encoder_output = (
                self._build_semantic_map_encoder_layers(
                    semantic_label_map_input
                )
            )
            generator_input = tf.concat(
                [intermediate_latents, semantic_label_encoder_output], axis=-1
            )
        else:
            ...
            if self._intermediate_conv_layer_kwargs is not None:
                for conv_layer_kwargs in self._intermediate_conv_layer_kwargs:
                    generator_input = self._conv_block(
                        generator_input, **conv_layer_kwargs
                    )

        w_bar = tf.pad(
            generator_input, [[0, 0], [1, 1], [1, 1], [0, 0]], "reflect"
        )

        upsampled = self._conv_block(
            w_bar,
            num_filters=self._num_filters_in_residual_blocks,
            kernel_size=3,
            strides=1,
            padding="valid",
        )
        ...
        res = self._residual_block(
            upsampled, self._num_filters_in_residual_blocks
        )
        for _ in range(self._num_residual_blocks - 1):
            res = self._residual_block(
                res, self._num_filters_in_residual_blocks
            )
        ...
        for num_filters in self._num_filters_per_layer_decoder:
            ups = self._upsample_block(
                ups, num_filters, 3, strides=[2, 2], padding="same"
            )

        ups = tf.pad(ups, [[0, 0], [3, 3], [3, 3], [0, 0]], "reflect")
        ups = tf.keras.layers.Conv2D(
            3, kernel_size=7, strides=1, padding="valid"
        )(ups)
    ...
    def _residual_block(
        ...
        padding_width = int((kernel_size - 1) / 2)  # type: ignore
        ...
        def _block_part(input_):
            ...
            res = tfa.layers.InstanceNormalization(
                **self._normalization_kwargs
            )(res)
            return res
        ...
        assert (
            res.get_shape().as_list() == identity_map.get_shape().as_list()
        ), ("Mismatched shapes between " "input/output!")
