...
class SelectiveDropout(keras.layers.Layer):
    ...
    def __init__(
        ...
        replacement_value: Callable = lambda input_shape: 0.0,
        ...
        assert 0.0 < ratio_of_never_dropped_elements <= 1.0
        self._ratio_of_never_dropped_elements = ratio_of_never_dropped_elements
        assert 0.0 < overall_probability_of_dropout < 1.0
    ...
    def call(self, inputs: tf.Tensor, *args, **kwargs) -> tf.Tensor:
        keep_mask = tf.cast(
            self._get_random_keep_mask_for_input(inputs), inputs.dtype
        )
        ...
        if self._replacement_value is None:
            ...
            def __dropped_out_inputs() -> tf.Tensor:
                replace_mask = tf.multiply(
                    tf.ones_like(inputs)
                    * self._replacement_value(tf.shape(inputs)),
                    1.0 - keep_mask,
                )
                return tf.add(tf.multiply(inputs, keep_mask), replace_mask)

        return keras.backend.in_train_phase(
            x=__dropped_out_inputs(), alt=inputs
        )
    ...
    def _get_random_keep_mask_for_input(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        if num_batches is not None:
            for batch_idx in range(num_batches):
                ...
                per_batch_keep_mask_shaped_like_input = tf.expand_dims(
                    tf.reshape(per_batch_keep_mask, inputs.shape[1:]), 0
                )
                keep_mask = tf.tensor_scatter_nd_update(
                    keep_mask,
                    [[batch_idx]],
                    per_batch_keep_mask_shaped_like_input,
                )
        return keep_mask

    def build(self, input_shape: tf.TensorShape) -> None:
        num_elements_per_batch = int(np.prod([*input_shape[1:]]))
        num_elements_never_dropped = int(
            round(
                self._ratio_of_never_dropped_elements
        ...
        if self._only_drop_full_channels:
            ...
            assert (
                num_elements_per_channel * num_channels
                == num_elements_per_batch
            )
            num_channels_to_always_keep = int(
                round(num_elements_never_dropped / num_elements_per_channel)
            )
            assert 1 <= num_channels_to_always_keep < num_channels

            probability_of_keeping_per_channel = np.linspace(
                1.0, 1.0 - self._overall_probability_of_dropout, num_channels
            )
            ...
            for channel_idx in range(num_channels):
                if channel_idx + 1 <= num_channels_to_always_keep:
                    ...
                    pmf[
                        :, :, channel_idx
                    ] = probability_of_keeping_per_channel[channel_idx]

        else:
            pmf = np.linspace(
                1.0,
                1.0 - self._overall_probability_of_dropout,
                num_elements_per_batch,
            )
            pmf[:num_elements_never_dropped] = 1.0
