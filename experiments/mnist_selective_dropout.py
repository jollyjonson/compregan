...
from basic_mnist_example import (
    HyperParameters as OriginalExperimentHyperParameters,
)
from basic_mnist_example import (
...
def build_dropout_quantizer() -> tf.keras.Model:
    selective_dropout_layer = SelectiveDropout(
        HyperParameters.ratio_of_never_dropped_indices,
...
if __name__ == "__main__":
    ...
    compgan_path = os.path.join(
        ...
        f"{HyperParameters.experiment_name}.pkl.tar.gz",
    )
    ...
    mnist_test_images_dataset = extract_and_preprocess_images(
        ...
        preprocess_func=lambda image: image / 255.0,  # [0, 255] -> [0, 1]
    ...
    if not HyperParameters.force_retrain_gan and os.path.exists(compgan_path):
        ...
        compression_gan = CompressionGAN(
            encoder=build_encoder(
                ...
                HyperParameters.use_conditional_gan,
                activation=None,
            ),
    ...
    for idx, image in tqdm.tqdm(enumerate(mnist_test_images_dataset)):
        ...
        for bit_idx, num_bits in enumerate(
            np.linspace(HyperParameters.num_bits, num_never_dropped_bits, 6)
        ):
            ...
            if HyperParameters.use_conditional_gan:
                bpp = (
                    num_bits + np.log2(HyperParameters.num_classes)
                ) / num_pixels
            else:
                bpp = num_bits / num_pixels

            print(
                f"{idx} @ {bpp if bit_idx != 0 else f'{bpp}_full_scale'}: ",
                latent_representation,
            )
            ...
            plt.savefig(
                os.path.join(
                    examples_dir,
                    str(idx) + f'_{str(bpp).replace(".", "_")}.png',
                )
            )
            if show_plots:
                plt.show()
