...
from compregan.losses.semantically_weighted_distortion import SemanticallyWeightedDistortion
...
from compregan.util import save_pip_freeze_to_file, debugger_connected, GlobalRandomSeed, copy_this_file_to_directory
...
class HyperParameters:
    ...
    num_filters_per_layer_encoder = (60, 120, 240, 480, 960) if not debug else (1, 3, 6)
    num_filters_per_layer_decoder = (480, 240, 120, 60) if not debug else (6, 6, 3)
...
dataset_preprocesing_kwargs = {'image_key': 'image_left',
                               'target_size': HyperParameters.img_shape,
                               'preprocess_func': lambda image: image / 255.,
                               'image_dataset_dtype': tf.float32}

cityscapes_train_orig = tfds.load('cityscapes', split='train')  # type: tf.data.Dataset
cityscapes_train_preprocessed = extract_and_preprocess_images(cityscapes_train_orig,
...
cityscapes_complete_orig = cityscapes_train_orig.concatenate(cityscapes_val_orig)
cityscapes_complete_preprocessed = cityscapes_train_preprocessed.concatenate(cityscapes_val_preprocessed)

label_getter = CityScapesBooleanSemanticMapGetter(cityscapes_complete_orig, cityscapes_complete_preprocessed)
...
compression_gan = CompressionGAN(
    ...
    loss_terms=(SemanticallyWeightedDistortion(100.),
                VGGPerceptualLoss(2., HyperParameters.img_shape),
...
compression_gan.train(
    ...
    shuffle=True,
    validation_data=None if HyperParameters.debug else cityscapes_val_preprocessed,
...
city_scapes_dataset_test = extract_and_preprocess_images(tfds.load('cityscapes', split='test'),
                                                         **dataset_preprocesing_kwargs)
