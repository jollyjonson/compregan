...
class DistortionTests(metaclass=ABCMeta):
    ...
    @abstractmethod
    def _expected_computation(self, original: tf.Tensor, reconstruction: tf.Tensor) -> Union[tf.Tensor, np.ndarray]:
        ...
    ...
    def test_distortion_delivers_correct_results_scalars(self):
        ...
        for original, reconstruction in zip(originals, reconstructions):
            ...
            distortion_value = self.distortion_instance(original_tf, reconstructed_tf)
            numpy.testing.assert_allclose(distortion_value, self._expected_computation(original, reconstruction))
    ...
    def test_distortion_delivers_correct_results_image_shaped_data(self):
        ...
        for image_shape in [self.image_shape, self.image_shape_with_batch_dim]:
            ...
            distortion_value = self.distortion_instance(original, reconstruction)
            numpy.testing.assert_allclose(distortion_value, self._expected_computation(original, reconstruction),
                                          atol=1e-8)

    def test_distortion_delivers_correct_needed_components(self):
        self.assertEqual((Components.OriginalData, Components.ReconstructedData),
    ...
    def test_multiplier_works(self):
        ...
        returned_gen_loss, returned_discr_loss = self.distortion_instance.apply(value, gen_loss, disc_loss)
...
class LpTests(DistortionTests, unittest.TestCase):
    ...
    def _expected_computation(self, original: tf.Tensor, reconstruction: tf.Tensor) -> Union[tf.Tensor, np.ndarray]:
        return np.power(np.sum(np.power(np.abs((original - reconstruction)), self.p)), 1. / self.p)
...
class MSSSIMTests(DistortionTests, unittest.TestCase):
    ...
    def _expected_computation(self, original: tf.Tensor, reconstruction: tf.Tensor) -> Union[tf.Tensor, np.ndarray]:
        return tf.image.ssim_multiscale(original, reconstruction, max_val=1.)
...
class PSNRTests(DistortionTests, unittest.TestCase):
    ...
    def _expected_computation(self, original: tf.Tensor, reconstruction: tf.Tensor) -> Union[tf.Tensor, np.ndarray]:
        return tf.image.psnr(original, reconstruction, max_val=1.)
