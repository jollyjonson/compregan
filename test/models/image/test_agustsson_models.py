...
class AgustssonModelsTest(unittest.TestCase):
    ...
    batch_size = 2
    latent_representation = np.random.randn(
        batch_size, *encoder.output_shape[1:]
    )
    ...
    def test_encoder_forward_pass(self):
        latent_representation = self.encoder(
            np.random.randn(self.batch_size, *self.encoder.input_shape[1:])
        )
        self.assertEqual(
            self.latent_representation.shape, latent_representation.shape
        )

    def test_decoding_generator_forward_pass(self):
        reconstructed_image = self.decoding_generator(
            self.latent_representation
        )
        self.assertEqual(
            reconstructed_image.shape, (self.batch_size, *self.image_shape)
        )

    def test_discriminator_forward_pass(self):
        discr_out = self.discriminator(
            np.random.randn(self.batch_size, *self.encoder.input_shape[1:])
        )
